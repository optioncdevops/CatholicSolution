...
**Unblock network access to a real database.** Every other piece of the Acutis auth contract layer — backend controller, JWT issuance/validation, response serialization, (Task 9) a full frontend integration, (Task 11) an automated test suite, and (Task 12) configuration scaffolding for repository switching — has been implemented and live-verified against the DEV FAKE repository (see the session's Task 1–13 reports). Three independent attempts (Tasks 8, 10, 13) to verify `NewViper.DoLogin` and replace `AcutisAuthenticationRepositoryDevFake` all stopped at the same class of problem: no database this session can actually reach has the required schema. Task 10 identified exactly which real database to target (the reference app's own `optionccom` development database); Task 13 confirmed that database's host is network-routable from here but its SQL Server port actively refuses connections. The next step is environmental, not code: either run the implementation work from a location with genuine network access to that database (or an equivalent, reachable one with the same contract), or have someone with access open the SQL Server port to this environment (firewall rule, VPN, tunnel, bastion host, etc.). Once a connection is genuinely possible, the remaining work is exactly what Tasks 8, 10, and 13 already scoped — verify the stored procedure's real contract, then implement the repository per [database-contract.md](database-contract.md) and register it by setting `AcutisAuth:RepositoryMode=Database` (Task 12's scaffolding is already in place for this). No further frontend work is blocked on this — the frontend (Task 9) is already fully wired to the real API shape and will work unchanged the moment the backend repository is real.
