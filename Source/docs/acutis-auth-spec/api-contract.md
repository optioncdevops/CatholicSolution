...
## Repository backing — real repository now implemented for Login (this session)

Five independent session attempts (Tasks 8, 10, 13, 15, and the post-Task 15 session task) to verify the real `NewViper.DoLogin` database contract all stopped at Phase 1 — see [database-contract.md](database-contract.md) for the full attempt history. **This session's sixth attempt succeeded**: a connection string was supplied, stored via User Secrets (never in a tracked file), and used to confirm `NewViper.DoLogin` live, including its parameters and both result-set shapes.

`AcutisAuthenticationRepository` (real, Dapper-backed) is now the **only** registered `IAcutisAuthenticationRepository` implementation — later in this same session, `AcutisAuthenticationRepositoryDevFake`, the `AcutisAuthenticationRepositoryNotImplemented` placeholder, and the `RepositoryMode` config switch were all removed entirely (real-time development only, per explicit instruction — no fake account exists anywhere in this codebase). `CFR.Acutis` now fails to start unless `ConnectionStrings:ConnString` is configured. **Only `Login` (`AuthenticateAsync`) is backed by a confirmed database object** — live-verified end-to-end through the full HTTP pipeline this session. `GET /navigation/menus` (`GetModuleRightsAsync`) and Forgot Password's account lookup (`FindAccountByEmailAsync`) still have no confirmed standalone database object and throw an explicit, honest `NotImplementedException` rather than guessing a query; Forgot Password's response stays enumeration-safe regardless (the service's existing exception handling covers this). Change/Reset Password's write step remains unsupported, unchanged, per this task's explicit instruction not to implement password writes. See [database-contract.md](database-contract.md#developmentfake-removed--real-time-development-only-this-session) for full detail.
