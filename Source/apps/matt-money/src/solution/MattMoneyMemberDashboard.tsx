...
export function MattMoneyMemberDashboard() {
  ...
  return (
    <>
      <DashboardHeader
        eyebrow="Billing & finance · Member"
        title="Member dashboard"
        status={<span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[10px] font-extrabold uppercase tracking-wide text-emerald-700">Account current</span>}
        actions={<button type="button" onClick={() => showToast('Statement download would start here')} className="action-secondary border border-emerald-200 bg-white text-emerald-700 hover:bg-emerald-50">Download statement</button>}
      />
