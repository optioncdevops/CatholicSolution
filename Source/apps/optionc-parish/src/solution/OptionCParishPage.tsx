...
const columns = [
  ...
  { title: 'Scheduled', color: 'bg-sky-500', cards: [['Confirmation', '24 candidates', 'Aug 25'], ['House blessing', 'D’Souza family', 'Aug 14']] },
...
export function OptionCParishPage() {
  ...
  return (
    <AppLayout app={app} className="parish-app">
      <main className="dashboard-content dashboard-stack">
        <DashboardHeader
          eyebrow="Parish administration"
          title="St. Joseph's Parish"
          description="Masses, sacraments, giving, and parish service requests in one workspace."
          status={<span className="portal-status-badge portal-status-badge--success">486 families active</span>}
          actions={<button type="button" onClick={() => showToast('Parish report would open here')} className="action-secondary">Parish report</button>}
        />

        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <KpiCard label="Registered families" value="486" detail="+6 new this month" accentClass="bg-emerald-500" icon="F" />
          <KpiCard label="Open requests" value="12" detail="2 need attention" accentClass="bg-emerald-600" icon="R" />
          <KpiCard label="Masses this week" value="11" detail="incl. 2 feast-day Masses" accentClass="bg-amber-500" icon="M" />
          <KpiCard label="Offertory · August" value="$86,500" detail="+12% vs July" accentClass="bg-sky-600" icon="$" />
          <KpiCard label="Sacraments YTD" value="73" detail="28 baptisms · 22 communions" accentClass="bg-slate-600" icon="S" />
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          ...
          <section className="surface-card p-5">
            ...
            <div className="mt-5 grid gap-4">{([['Sunday offertory', '$52,300', 82, 'bg-emerald-500'], ['Online giving', '$24,700', 46, 'bg-teal-500'], ['Candles & intentions', '$9,500', 22, 'bg-amber-500']] as const).map(([label, value, width, color]) => <div key={label}><div className="mb-2 flex justify-between text-xs font-bold"><span className="text-slate-700">{label}</span><span className="text-slate-400">{value}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${color}`} style={{ width: `${width}%` }} /></div></div>)}</div>
            <p className="mt-4 rounded-xl border border-dashed border-emerald-100 bg-emerald-50/50 p-3 text-[11px] leading-5 text-slate-500">Building-fund pledge drive is at 68% of its $500K goal. Online giving is up 21% since the parish app launch.</p>
          </section>
        </div>
        ...
        <section className="surface-card p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">{([['B', 'Rodrigues family', 'Baptism · Aug 02 · Fr. Robert'], ['I', 'Almeida', 'Mass intention · Aug 03 · Recorded'], ['M', "D'Cruz & Nayak", 'Marriage · Jul 26 · Certificate issued']] as const).map(([icon, name, detail]) => <article key={name} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><span className="grid size-9 place-items-center rounded-lg bg-emerald-100 text-xs font-extrabold text-emerald-800">{icon}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-800">{name}</strong><span className="text-xs text-slate-400">{detail}</span></span><button type="button" onClick={() => showToast(`Certificate for ${name} would open here`)} className="rounded-lg border border-emerald-100 bg-emerald-50 px-2.5 py-1.5 text-[10px] font-extrabold text-emerald-700 hover:bg-emerald-100">Certificate</button></article>)}</div>
        </section>
        ...
        <section>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2 2xl:grid-cols-4">{columns.map((column) => <div key={column.title} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-[var(--shadow-soft)]"><div className="flex items-center justify-between px-1 pb-3"><h3 className="text-xs font-extrabold uppercase tracking-wide text-slate-700"><span className={`mr-2 inline-block size-2 rounded-full ${column.color}`} />{column.title}</h3><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-extrabold text-slate-600">{column.cards.length}</span></div>{column.cards.map(([title, owner, due]) => <button key={title} type="button" onClick={() => showToast(`${title} details would open here`)} className="mb-2.5 w-full rounded-xl border border-slate-200 bg-white p-4 text-left hover:border-emerald-200 hover:shadow-[var(--shadow-soft)]"><strong className="block text-sm text-slate-800">{title}</strong><span className="mt-1 block text-xs text-slate-400">{owner}</span><span className="mt-3 inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-extrabold text-emerald-700">Due {due}</span></button>)}</div>)}</div>
        </section>
      </main>
    </AppLayout>
  );
