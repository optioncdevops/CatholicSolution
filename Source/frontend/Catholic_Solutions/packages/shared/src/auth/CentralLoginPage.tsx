...
export function CentralLoginPage() {
  ...
  const requiresInteractiveSignIn = useMemo(() => {
    ...
    return (clientId === 'platform' || clientId === 'platform-admin') && params.get('entry') === 'platform';
  ...
  const completeSignIn = async (provider: 'password' | 'google' | 'microsoft') => {
    ...
    if (result === 'authenticated') {
      ...
      showToast(clientId === 'platform' ? 'Signed in to Catholic Solutions' : clientId === 'platform-admin' ? 'Signed in to Catholic Solutions Admin' : `Signed in. Returning to ${client.name}`);
