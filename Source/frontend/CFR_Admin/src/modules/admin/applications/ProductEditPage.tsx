...
export function ProductEditPage() {
  ...
  return (
    <div className="admin-reveal flex flex-col gap-4">
      <PanelHeader breadcrumb={{ label: app.name, to: detailPath }} title="Edit product" />
      ...
      <form onSubmit={(event) => { event.preventDefault(); handleSave(); }} noValidate className="flex flex-col gap-4">
        ...
        <div className="admin-sticky-footer">
          ...
          <CommonButton variant="primary" iconLeft={<Save size={14} />} onClick={handleSave} disabled={hasErrors}>Save changes</CommonButton>
        </div>
      </form>
    </div>
  );
