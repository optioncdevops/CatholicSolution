...
namespace CFR.AcutisService.Service.AcutisAuthentication
{
        ...
        ILogger<AcutisPasswordService> logger): IAcutisPasswordService
    {
        ...
        private async Task SendResetEmailAsync(ForgotPasswordUserResult user, string rawToken)
        {
            ...
            // Fallback kept in sync with the PasswordReset seed in 006_Acutis_EmailTemplates.sql (and its
            // 008_Acutis_EmailTemplates_PasswordResetRedesign.sql upgrade) — this only fires when the
            // admin-configurable template row is missing entirely.
