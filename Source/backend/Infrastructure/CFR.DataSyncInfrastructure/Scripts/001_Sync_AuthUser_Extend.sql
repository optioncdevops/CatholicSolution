...
-- CFR.DataSync Phase 1 — extends [auth].[User] for the product/user sync API.
...
--   2. Makes Password nullable — CFR.DataSync creates auth.User rows for members it has
...
--      every existing script that touches this table); CFR.DataSync needs them to track
...
-- 2. Password becomes nullable — CFR.DataSync-created identities have no password until
