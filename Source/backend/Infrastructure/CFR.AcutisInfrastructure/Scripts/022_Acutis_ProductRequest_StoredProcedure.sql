...
-- Requires 021_Acutis_ProductRequest_Tables.sql and CFR.DataSync's 004_Sync_Security_Schema.sql
-- ([sec].[ApiClient]) to have already been run. Run 026_Acutis_Product_Drop_SecurityKey.sql
-- after this script to drop the no-longer-used [core].[Product].[SecurityKey] column.
--
...
--               [core].[ProductEnvironment] (Development/Staging/Pilot/Live + public website row,
--               all set to the submitted Production URL),
--               register the product's CFR.DataSync login credential in [sec].[ApiClient]
--               (RateLimitPerMinute 6000, MaxBulkUserCount 1000),
--               mark the request approved, link it to the new ProductId. @ClientSecret is
--               generated by the caller (ProductRequestService.ApproveProductRequestAsync,
--               CFR.AcutisService) and saved into [sec].[ApiClient].[ClientSecret] as-is (that
--               column is plaintext - see 006_Sync_StoredProcedures.sql), NOT onto core.Product.
--               The generated ClientId ("<product-name>-client", same shape as the existing
--               sms-client / parish-hub-client rows) is returned via @ClientId OUTPUT.
...
CREATE PROCEDURE [dbo].[Acutis_ProductRequest]
    ...
    @ClientSecret NVARCHAR(200) = NULL, -- ActionId 4 only: generated by the caller, saved into [sec].[ApiClient]
    @ClientId NVARCHAR(100) = NULL OUTPUT, -- ActionId 4 only: the [sec].[ApiClient].[ClientId] created for the new product
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @ClientIdBase NVARCHAR(80);

        SET @ClientSecret = NULLIF(LTRIM(RTRIM(@ClientSecret)), N'');
        ...
        BEGIN TRY
            ...
            -- ClientId is unique ([UX_ApiClient_ClientId]); fall back to appending the new
            -- ProductId when another client already owns "<product-name>-client".
            SET @ClientIdBase = LEFT(LOWER(REPLACE(LTRIM(RTRIM(@ApproveProductName)), N' ', N'-')), 80);
            SET @ClientId = @ClientIdBase + N'-client';

            IF EXISTS (SELECT 1 FROM [sec].[ApiClient] WITH (UPDLOCK, HOLDLOCK) WHERE [ClientId] = @ClientId)
            BEGIN
                SET @ClientId = @ClientIdBase + N'-' + CAST(@NewProductId AS NVARCHAR(10)) + N'-client';
            END

            -- Default limits for a newly approved product: 6000 requests/minute and up to
            -- 1000 users per BulkCreateUsers call (same values as the column defaults).
            ...
            VALUES
            (
                @ClientId, @ClientSecret, @NewProductId, @ApproveProductName, 6000, 1000, CONVERT(NVARCHAR(100), @UpdatedBy)
            );
            ...
            -- One [core].[ProductEnvironment] row per environment for the new product, all pointing at
            -- the Production URL the requester submitted (an admin can re-point each environment later
            -- from the product's API integration settings), plus the product's public website row -
            -- EnvironmentName NULL, the same convention the existing products use (e.g. SMS's
            -- "Official ... public website" row). EnvironmentName values follow
            -- CK_ProductEnvironment_EnvironmentName (Development / Staging / Pilot / Live - QA maps to
            -- Pilot and Production to Live, see 004_Portal_ProductEnvironment_Names.sql).
            IF NULLIF(LTRIM(RTRIM(@ApproveExternalPageUrl)), N'') IS NOT NULL
            BEGIN
                INSERT INTO [core].[ProductEnvironment]
                (
                    [ProductId], [EnvironmentName], [BaseUrl], [Description], [IsActive], [CreatedDate], [InsertedBy], [IsDeleted]
                )
                SELECT
                    @NewProductId, e.[EnvironmentName], LTRIM(RTRIM(@ApproveExternalPageUrl)), e.[Description], 1, SYSUTCDATETIME(), @UpdatedBy, 0
                FROM (VALUES
                    (N'Development', N'Local development environment used for development and testing.'),
                    (N'Staging', N'Staging environment used for integration and pre-release testing.'),
                    (N'Pilot', N'Pilot environment used for pilot deployment and validation.'),
                    (N'Live', N'Live production environment.'),
                    (NULL, N'Official ' + @ApproveProductName + N' public website.')
                ) AS e ([EnvironmentName], [Description])
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM [core].[ProductEnvironment] pe
                    WHERE pe.[ProductId] = @NewProductId
                      AND pe.[IsDeleted] = 0
                      AND ISNULL(pe.[EnvironmentName], N'') = ISNULL(e.[EnvironmentName], N'')
                );
            END
