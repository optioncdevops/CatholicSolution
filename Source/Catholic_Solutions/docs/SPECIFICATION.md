# Catholic Solutions — Living Specification v2.0.3

> **Canonical release baseline:** v1.0.0. **Current release:** v2.0.3. Architecture changes are cumulative; where a historical section conflicts with §51, §51 and this current-architecture preface take precedence.
...
This is the only maintained specification document for the current implementation. Every approved behavior, UI standard, architecture change, catalog change, validation result, or scope change must update this file in the same delivery as the code.
...
5. The current repository source, validated against the newest applicable specification section.

Precedence is newest explicit user direction first. A newer approved direction may supersede an older specification behavior. When that happens, source and this Living Specification must be updated together.

## 2. Product purpose and flow

Catholic Solutions now operates as a **SaaS control plane** with two internally owned applications: CFR for end-user access/App Hub and Super Admin for platform operations. The App Registry and Universal App Switcher are shared ecosystem infrastructure; business products remain independently deployed.

The repository owns:

- CFR / Catholic Solutions Login and Request Access entry points.
- CFR App Hub / application discovery and launch/request experience.
- Super Admin platform-operations frontend.
- Shared internal authentication/design-system support used by CFR and Super Admin.
- A framework-neutral application registry consumed by CFR, Super Admin, and platform integrations.
- A separately buildable, domain-neutral Catholic Solutions App Switcher that can be hosted on an approved platform/CDN domain.

Connected products such as OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center, and approved partner SaaS products are independently deployed applications. Their product source repositories do **not** belong inside this repository merely to participate in App Hub or the App Switcher.

Primary platform flow:

`Login → App Hub → Launch/request connected app → Connected app uses hosted App Switcher → App Hub / another approved app`
...
- React Router
- npm workspaces
- Two deployable platform frontends in this repository: CFR and Super Admin
- One internal shared React package for CFR/Admin
- One framework-neutral App Registry package with no React dependency
- One independently buildable App Switcher package based on browser Web Components / Custom Elements and Shadow DOM
...
Catholic_Solution/
├── apps/
│   ├── cfr/                       # end-user CFR / App Hub
│   │   ├── Login
│   │   ├── Request Access
│   │   └── App Hub
│   └── platform-admin/            # Super Admin control plane
├── packages/
│   ├── app-registry/              # framework-neutral catalog + launch metadata
│   ├── shared/                    # CFR-owned React/auth/design-system support
│   └── app-switcher/              # universal browser integration + independent build
├── config/
│   └── solutions.json             # machine-readable CFR repository boundary
├── docs/
│   └── integrations/              # external-team App Switcher integration contract
└── scripts/
    └── build-staging.mjs
```
...
- `apps/cfr` and `apps/platform-admin` are the only application sources under `apps/*` in this repository.
- Other Catholic Solutions and partner applications are represented only by approved catalog metadata and destinations.
- Do not add another product workspace under `apps/*` solely to make that product appear in App Hub or the launcher.
- `packages/app-registry` is the single framework-neutral application metadata source for CFR App Hub and launcher publication.
- `packages/shared` contains code used by CFR itself; it is no longer a mechanism for distributing a React shell into every connected product repository.
- `packages/app-switcher` is browser-framework-neutral and must not depend on React, Tailwind, Angular, Vue, Bootstrap, or a consuming application's source tree.
- `config/solutions.json` must describe only deployable solutions actually owned by this repository.

### 3.2 Central application catalog

`packages/app-registry/src/appCatalog.ts` is the single source of truth for App Hub section membership, approved destinations, and launcher publication. `packages/shared/src/app/config/appCatalog.ts` is a compatibility re-export for CFR components only.

Each catalog record owns its own:

- stable `id`;
- display metadata;
- `hubSection: 'your' | 'available' | 'future'`;
- approved `externalUrl` when a real destination exists;
- `navigationTarget`;
- `launcherEnabled` publication flag;
- truthful catalog status and detail metadata.

App Hub collections and the public App Switcher catalog must be derived from these record-level fields. Separate hardcoded ID sets or product-local switcher lists are not permitted.

### 3.3 Independent application model

Connected application source may live in any independently owned repository or technology stack, for example React, Angular, Vue, ASP.NET, Java, PHP, or plain HTML.

A connected app requires no source-code dependency on Catholic Solutions. Participation in the launcher requires only:

1. central Catholic Solutions catalog registration; and
2. the hosted browser integration in the consuming application.

Authentication, SSO, provisioning, and federation are separate integration concerns. Being listed in App Hub or the App Switcher does not imply shared credentials, cookies, tokens, or session state.

### 3.4 CFR domain and environment

Development CFR origin: `http://localhost:4001`  
Production CFR origin: `https://cfr.optioncapp.com`

CFR contains `.env.development` and `.env.production` with public build metadata only. Authentication mode and CFR origin remain centralized in `packages/shared/src/auth/appAuthConfig.ts`. Secrets must never be stored in `VITE_*` values.

Product destination URLs belong to the central application catalog, not to an internal multi-app domain registry.

### 3.5 Cross-application navigation

- App Hub, Details, and the hosted App Switcher use the same approved catalog destination.
- Production product destinations must use HTTPS.
- Same-tab is the default launch behavior unless `navigationTarget: 'new-tab'` is explicitly configured.
- New-tab launches must use `noopener noreferrer`.
- Future products without real destinations remain non-launchable.
- The current application is inert in the App Switcher and is identified only by its stable catalog ID.
- `All apps in App Hub` navigates to CFR `/apps`.

### 3.6 Framework-neutral App Switcher contract

The stable v1 browser integration is built independently from CFR and deployed to an approved Catholic Solutions platform/CDN origin:

`https://<platform-or-cdn-domain>/app-switcher/v1/app-switcher.js`

A consuming application integrates it with only:
...
The consuming application must not maintain a duplicate application list. The Web Component loads sibling `catalog.js` from the script host by default. The catalog's App Hub destination is configured independently, so the launcher host does not need to be the CFR domain.

The v1 path is the public integration contract. Normal backward-compatible fixes remain under `/v1/`; a future `/v2/` path is required only for an intentionally breaking integration contract.

### 3.7 App Switcher web-platform requirements

- Implement as an autonomous Custom Element registered as `catholic-solutions-app-switcher`.
- Use Shadow DOM for style isolation from host frameworks and CSS.
- Runtime code must use safe DOM APIs/text assignment for catalog-provided labels and must not inject product strings as HTML.
- Current-app tile is non-navigating and does not gain actionable hover state.
- Other tiles retain normal focus, keyboard, hover, same-tab/new-tab, and anchor semantics.
- The hosted component must expose no React requirement to consuming applications.
- React + TypeScript consumers may add the optional declaration from `packages/app-switcher/react.d.ts`; it is compile-time typing only and not a runtime dependency.
- Partner CSP must permit the approved platform/CDN runtime origin in `script-src`; a separately hosted catalog origin must also be allowed.
- The runtime must not assume or require a CFR hostname.

### 3.8 Central update propagation

The independent platform build publishes:

- `/app-switcher/v1/app-switcher.js` — component/runtime behavior.
- `/app-switcher/v1/catalog.js` — centrally derived approved application catalog.

`catalog.js` is requested by the component using a short cache bucket and platform/CDN hosting applies a short revalidation policy to catalog data. Normal catalog additions/removals/destination corrections therefore propagate to connected applications without copying source or changing partner markup.

CFR and the platform assets are separate deployable outputs. A CFR application rebuild is not required merely to publish a launcher/catalog change, and an external partner application rebuild is not required for a normal central catalog update.

## 4. Catalog scope

The App Hub has exactly three visible sections derived from each catalog record's `hubSection`:
...
These are the organization's subscribed/owned applications and retain the strongest card emphasis with Launch + Details behavior.
...
These are requestable/available connected applications. Request access is primary and Details is secondary; where an approved destination exists, the existing logo may act as the direct external/demo destination according to the approved App Hub behavior.

### Future Apps

All remaining catalog records are derived as `hubSection: 'future'`. Future cards use quieter presentation and must not visually claim Active status merely because historical prototype metadata contained an active operational status.

### App Switcher publication

The App Switcher is independent from App Hub section membership. It publishes only records satisfying both:

1. `launcherEnabled: true`; and
2. a valid approved production `externalUrl`.

This allows an approved connected application to appear in the launcher even when App Hub currently classifies it under Future Apps, while products without a real destination remain absent. Launcher publication never implies repository ownership or shared authentication.

Current centrally configured production destinations include OptionC School, Matt Money, ArcAlerts, Parish Hub, Catholic Content, Unified Directory, Support Center, AI Lesson Plan/XtraCoach, FerrerWorks, Mass Card Requests, Vincent Volunteer, Berchmans, Alive Date, and Friar Friend.

The App Hub, Details modal, and hosted App Switcher must use the same centralized typed catalog and canonical destination resolver/build projection rather than duplicating product metadata or URLs.
...
## 47. v1.7.0 — CFR-only repository and platform-neutral App Switcher architecture

**Date:** August 19, 2026. This is an intentional architecture restructuring. It supersedes the repository-ownership portions of §§3, 6, 10, 45, and 46 where those sections describe every connected product as source under `apps/*` or describe React shared-shell propagation as the long-term launcher distribution model. Historical behavior records remain in this Living Specification for traceability, but §47 and the current architecture preface are authoritative.

### 47.1 Architecture requirement

- **Owner:** Platform / Frontend Engineering.
- Catholic Solutions MUST contain the complete CFR/platform frontend boundary only.
- `apps/cfr` is the only deployable frontend application owned by this repository and owns Login, Request Access, and App Hub.
- OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center, AI Lesson Plan, and partner products MUST NOT require source folders in this repository merely to participate in the ecosystem.
- Connected products may use independent repositories, deployments, release cycles, and frontend/server technology stacks.
- `config/solutions.json` MUST list only actual repository-owned deployment boundaries.
- A connected product's presence in App Hub or App Switcher MUST be driven by catalog metadata, not by the existence of an `apps/<product>` folder.

### 47.2 Central catalog contract

`packages/shared/src/app/config/appCatalog.ts` remains the canonical product catalog. Each record owns its own App Hub and launcher classification through:

- `hubSection` — `your`, `available`, or `future`;
- `launcherEnabled` — explicit approval to publish in the launcher;
- `externalUrl` — independently deployed approved destination when launchable;
- `navigationTarget` — same-tab/new-tab behavior.

`yourApps`, `availableApps`, `futureApps`, and `availableSwitcherApps` MUST be derived from these fields. Separate ID sets are prohibited.

All formerly repository-owned product destinations are represented as approved external deployment URLs. CFR's `SOLUTION_REGISTRY` contains only the CFR/platform solution and is not a product destination map.

### 47.3 Framework-neutral launcher package

The browser integration source lives under:

`packages/app-switcher`

It MUST remain independent of React, React Router, Tailwind, Bootstrap, Angular, Vue, or any consuming application framework.

The public runtime is an autonomous Custom Element:

`catholic-solutions-app-switcher`

and uses Shadow DOM for presentation isolation. The consumer contract is one hosted script plus one HTML element:

```html
<script src="https://cfr.optioncapp.com/integrations/app-switcher/v1/app-switcher.js" defer></script>
<catholic-solutions-app-switcher current-app-id="<assigned-app-id>"></catholic-solutions-app-switcher>
```

No consumer-side application catalog, React package import, CSS import, or Catholic Solutions repository access is required.

### 47.4 Runtime/catalog separation

CFR builds MUST publish:

- `/integrations/app-switcher/v1/app-switcher.js` — stable v1 runtime/UI contract.
- `/integrations/app-switcher/v1/catalog.js` — central launcher catalog projection.

The runtime automatically loads sibling `catalog.js`. This separation allows catalog updates to propagate without embedding the full product list into consumer source or requiring partner markup changes.

The runtime uses a short cache-bucket query for `catalog.js`; supported CFR hosting configuration applies short revalidation headers to the integration path.

The previous unversioned paths are retained as compatibility aliases during the v1 migration:

- `/integrations/app-switcher/app-switcher.js`
- `/integrations/app-switcher/catalog.js`

New integrations MUST use `/v1/`.

### 47.5 Web-platform and security requirements

1. Runtime registration uses the browser Custom Elements registry and MUST be idempotent.
2. Shadow DOM isolates component presentation from consuming-framework styles.
3. Product labels/categories MUST be written through safe text DOM APIs, not raw HTML injection.
4. Production catalog projection accepts only HTTPS product destinations.
5. The consuming page cannot provide arbitrary launcher destinations.
6. Explicit new-tab destinations use `noopener noreferrer`.
7. The current app is identified by stable ID, rendered inert, and receives no actionable hover state or CURRENT/check badge.
8. Other applications retain anchor semantics, focus, keyboard, hover, and configured tab behavior.
9. The launcher panel MUST position against the viewport so placement in a left-, center-, or right-aligned host header does not clip the panel off-screen; it MUST recompute on resize/scroll and may open above the trigger when lower viewport space is insufficient.
10. `All apps in App Hub` resolves to CFR `/apps`.
11. The component performs navigation only; SSO/federation/token/cookie exchange remains separate identity work.
12. A restrictive partner CSP must permit the CFR integration origin in `script-src` because the runtime loads the central catalog script from the same origin.

### 47.6 React + TypeScript developer experience

React/TSX consumers still use the same browser runtime. They may add the optional declaration maintained at:

`packages/app-switcher/react.d.ts`

to their own project, normally:

`src/types/catholic-solutions-app-switcher.d.ts`

This declaration exists only to extend React JSX typing and MUST NOT become a runtime package dependency.

### 47.7 CFR build/publishing boundary

- `apps/cfr/vite.config.ts` owns publication of versioned and compatibility App Switcher runtime/catalog assets.
- `apps/cfr/tsconfig.node.json` explicitly includes the App Switcher manifest builder and its direct catalog/auth/type dependencies so Node/Vite build-time TypeScript remains inside the project boundary.
- `apps/cfr/netlify.toml`, `apps/cfr/vercel.json`, `apps/cfr/public/web.config`, and `apps/cfr/public/.htaccess` apply equivalent short revalidation/integration-asset headers for supported hosts.
- The former App Hub workspace is renamed to `apps/cfr` and package name `@catholic-solutions/cfr` to reflect the repository boundary.
- Old product workspace build/dev scripts are removed from the root package contract.

### 47.8 Source removals and ownership transition

The following product workspaces are intentionally removed from Catholic_Solution source ownership:
...
Their catalog identities and approved destinations remain available to CFR. Removing their source folders does not remove them from the Catholic Solutions ecosystem.

The old internal React launcher distribution files (`AppLayout`/`PlatformTopbar`/`PlatformAppSwitcher` aliases and related product-layout plumbing) are no longer the cross-application integration mechanism and are removed when unused by CFR.

### 47.9 Implementation traceability

- `apps/cfr/*` — only deployable repository-owned frontend.
- `packages/shared/src/app/config/appCatalog.ts` — central record-level App Hub/launcher classification and destinations.
- `packages/shared/src/app/types/app.ts` — `hubSection`, `launcherEnabled`, destination/launch contract.
- `packages/shared/src/platform/navigation/solutionNavigation.ts` — CFR App Hub destination validation and navigation.
- `packages/shared/src/auth/appAuthConfig.ts` — CFR-only platform/auth origin configuration.
- `packages/shared/src/platform/config/solutionRegistry.ts` — CFR/platform-only solution registry.
- `packages/app-switcher/src/app-switcher.js` — framework-neutral Web Component runtime.
- `packages/app-switcher/src/manifest.ts` — public HTTPS-only launcher catalog projection.
- `packages/app-switcher/react.d.ts` — optional React/TypeScript JSX declaration.
- `apps/cfr/vite.config.ts` — runtime/catalog publishing and compatibility aliases.
- `config/solutions.json` — CFR-only deployment manifest.
- `docs/integrations/APP_SWITCHER.md` — cross-platform external-team integration guide.
- `README.md` — current repository boundary and developer entry point.

### 47.10 Acceptance criteria

1. `apps/*` contains only `apps/cfr`.
2. Root scripts build/dev/typecheck only the CFR application; removed product workspace commands no longer exist.
3. App Hub still derives Your Apps / Available Apps / Future Apps from the centralized catalog.
4. Section membership is stored on each catalog record; no separate `yourAppIds` / `availableAppIds` sets remain.
5. Every launcher-published product has `launcherEnabled: true` and a valid HTTPS destination.
6. Production App Switcher catalog projection excludes records without valid approved HTTPS destinations.
7. A consuming plain HTML application can use one script tag and one custom element with no React/CSS package dependency.
8. React/TSX consumers have a documented optional `.d.ts` fix for JSX custom-element typing.
9. Current-app behavior remains inert and all other approved destinations remain navigable.
10. New-tab destinations preserve `noopener noreferrer`.
11. CFR publishes both `/v1/` runtime/catalog assets and the temporary unversioned compatibility aliases.
12. `config/solutions.json`, root package metadata, package lock root metadata, README, developer guide, and Living Specification are synchronized to v1.7.0.
13. SSO/federation is not coupled to launcher registration.
14. Launcher placement is viewport-aware and does not assume the host application places the trigger at the right edge of its header.

### 47.11 Validation record

Validation for the delivered v1.7.0 source confirms:

- `apps/*` contains exactly one deployable workspace: `apps/cfr`.
- The centralized catalog contains 18 records: 2 Your Apps, 6 Available Apps, and 10 Future Apps.
- 14 records are launcher-enabled and every launcher-enabled record has an approved HTTPS destination; the public catalog projection emits 14 entries and zero non-HTTPS destinations.
- The public projection resolves App Hub to `https://cfr.optioncapp.com/apps` and serializes through the v1 catalog global consumed by the browser runtime.
- `packages/app-switcher/src/app-switcher.js` passes Node JavaScript syntax validation.
- 41 repository TypeScript/TSX source files pass parser validation with zero parser diagnostics.
- `apps/cfr/tsconfig.node.json` produces zero `TS6307` project-boundary diagnostics in targeted validation with dependency-only module shims; the manifest/catalog/auth/type dependency chain is explicitly listed.
- JSON deployment/package metadata and IIS `web.config` parse successfully; supported-host configuration covers the integration asset path.
- The runtime contains no embedded product list and loads the centrally published sibling `catalog.js`.
- The final source ZIP is integrity-tested before delivery.

A dependency-backed Vite production build could not be completed in this execution environment because installed workspace dependencies were unavailable and npm's offline cache did not contain all required packages. Run `npm install` followed by `npm run build:production` in the normal development/CI environment as the final dependency-backed build gate.


## 48. v1.8.0 — CFR application + domain-neutral platform integration architecture

**Date:** August 19, 2026. This section supersedes §47 wherever the older section makes CFR the publication/hosting boundary for the Universal App Switcher or makes `packages/shared` the canonical application catalog.

### 48.1 Architectural requirement

Catholic Solutions MUST separate application ownership from platform integration ownership:

- **CFR is an application**, owning Login, Request Access, App Hub, and CFR-specific UI/auth behavior.
- **The Application Registry is platform metadata**, not React application code.
- **The Universal App Switcher is platform integration infrastructure**, not a CFR-bundled feature.
- Connected Catholic Solutions and partner products MAY live in separate repositories, organizations, domains, hosting environments, and technology stacks.
- A product MUST NOT require an `apps/*` folder inside Catholic_Solution merely to appear in App Hub or the launcher.

The target relationship is:

`App Registry -> Universal App Switcher publication -> platform/CDN host -> CFR + independent Catholic Solutions apps + external partner apps`

### 48.2 Repository boundary

The current repository owns:

```text
apps/cfr/                       CFR application only
packages/app-registry/          framework-neutral app metadata and launch contract
packages/app-switcher/          framework-neutral Custom Element and independent platform build
packages/shared/                CFR React/auth/design-system support
config/solutions.json           repository/deployment boundary
docs/                           living specification and integration contracts
```

Rules:

1. `apps/*` MUST contain only repository-owned applications. For this release, that is `apps/cfr` only.
2. `packages/shared` MUST NOT be used as the distribution mechanism for partner application shells.
3. Registry data MUST NOT depend on React, React Router, Tailwind, CFR routes, or the App Switcher UI.
4. The App Switcher MUST consume the registry projection without owning a second application list.
5. External product source remains outside this repository unless the product is explicitly brought under repository ownership by a future architecture decision.

### 48.3 Framework-neutral application registry

Canonical source:

`packages/app-registry/src/appCatalog.ts`

Canonical types:

`packages/app-registry/src/types.ts`

Each catalog record owns:

- stable application ID;
- display name/metadata;
- `hubSection` (`your`, `available`, `future`);
- approved `externalUrl` when a real destination exists;
- `launcherEnabled` publication approval;
- `navigationTarget` (`same-tab`, `new-tab`);
- truthful status/details used by CFR App Hub.

`packages/shared/src/app/config/appCatalog.ts` and `packages/shared/src/app/types/app.ts` MAY remain as CFR compatibility re-exports, but MUST NOT become competing catalog/type sources.

### 48.4 Independent App Switcher build boundary

`packages/app-switcher` MUST have its own production build independent from `apps/cfr`.

Required root commands:

```text
npm run build:platform       -> packages/app-switcher/dist/app-switcher/v1/*
npm run build:cfr            -> CFR Vite build
npm run build:production     -> platform assets + CFR + migration-only CFR compatibility bridge
```

Required platform output:

```text
app-switcher/v1/app-switcher.js
app-switcher/v1/catalog.js
```

`apps/cfr/vite.config.ts` MUST NOT import the App Switcher manifest builder or publish the launcher catalog as part of the CFR Vite bundle. This removes the previous CFR/App-Switcher TypeScript project coupling and eliminates the need for CFR `tsconfig.node.json` to enumerate App Switcher/catalog build-time source files.

### 48.5 Domain-neutral hosting contract

The App Switcher runtime MUST NOT assume that its host equals the CFR application domain.

Normal deployment model:

```text
https://<approved-platform-or-cdn-domain>/app-switcher/v1/app-switcher.js
https://<approved-platform-or-cdn-domain>/app-switcher/v1/catalog.js
```

The final production platform/CDN hostname is deployment configuration and MUST NOT require changes to the Web Component source.

Runtime rules:

1. `app-switcher.js` loads sibling `catalog.js` relative to its own script URL by default.
2. The published catalog contains the configured CFR/App Hub URL independently of the script host.
3. Advanced infrastructure MAY provide `data-catalog-url` on the script when catalog and runtime assets are hosted separately.
4. The element MAY provide `app-hub-url` as an infrastructure override.
5. Normal partner integration remains one script reference plus one element; advanced overrides are optional.

### 48.6 Public integration contract

The v1 consumer contract remains intentionally framework-neutral:
...
External teams MUST NOT need:

- Catholic Solutions repository access;
- CFR source or CFR deployment artifacts;
- React/Tailwind/shared-package installation;
- a copied application list;
- knowledge of other application destination URLs.

The current app is identified by its stable assigned ID and remains inert. Other approved launcher destinations retain configured navigation behavior.

### 48.7 React + TypeScript contract

React/TSX remains a consumer of the same browser component. `packages/app-switcher/react.d.ts` is an optional compile-time declaration only and supports:
...
External React teams normally copy this declaration to:

`src/types/catholic-solutions-app-switcher.d.ts`

No React runtime package from Catholic Solutions is required.

### 48.8 Authentication / cross-domain session boundary

The App Switcher performs navigation only and MUST NOT transport credentials, access tokens, refresh tokens, cookies, or implicit SSO state.

Because connected applications may live on unrelated domains:

1. CFR preview/production session cookies MUST be host-scoped by default; broad `.optioncapp.com` cookie sharing is not the ecosystem SSO mechanism.
2. External applications own their own local application session.
3. Cross-domain SSO/federation is a separate identity integration and SHOULD use the platform-approved identity provider/federation protocol (for example OIDC) rather than shared browser cookies.
4. Launcher registration and SSO registration are separate approvals.
5. `docs/integrations/SSO_FEDERATION.md` documents this boundary; implementing a production identity-provider backend is outside the prototype frontend scope unless explicitly requested.

### 48.9 CFR hosting boundary

Primary App Switcher CDN/CORS/cache policy belongs to the App Switcher/platform hosting deployment. `packages/app-switcher/hosting/` provides host examples for the platform assets.

For backward compatibility only, the root production build copies the already-built platform runtime/catalog into CFR `dist/integrations/app-switcher/*`, and CFR host configs retain short-revalidation/CORS headers for that migration path. CFR does not compile the registry/manifest and is not the primary launcher publication boundary. New integrations must use the neutral platform/CDN host.

### 48.10 Security requirements

1. Production registry projection MUST publish only approved HTTPS destinations.
2. The consuming application MUST NOT supply or mutate the central application list.
3. New-tab destinations MUST use `noopener noreferrer`.
4. Product labels/categories MUST be rendered through safe text DOM APIs.
5. The Web Component MUST remain Shadow DOM isolated from consuming application CSS.
6. A restrictive consumer CSP MUST explicitly allow the approved App Switcher runtime/catalog script origin(s).
7. Registry/catalog publication MUST contain navigation metadata only; no user/session/credential data is permitted.
8. The runtime must remain idempotent when referenced more than once.

### 48.11 Implementation traceability

- `apps/cfr/vite.config.ts` — CFR-only Vite build; no App Switcher publisher/import chain.
- `scripts/copy-legacy-switcher-bridge.mjs` — post-build migration bridge copying already-built platform assets to previous CFR integration paths; no catalog compilation.
- `apps/cfr/tsconfig.node.json` — CFR build config boundary reduced to CFR Vite configuration only.
- `packages/app-registry/src/appCatalog.ts` — canonical cross-domain product registry.
- `packages/app-registry/src/types.ts` — framework-neutral catalog contract.
- `packages/shared/src/app/config/appCatalog.ts` — CFR compatibility re-export only.
- `packages/shared/src/app/types/app.ts` — CFR compatibility type re-export only.
- `packages/app-switcher/src/app-switcher.js` — universal Custom Element; catalog URL resolved from script host with optional infrastructure override.
- `packages/app-switcher/src/manifest.ts` — HTTPS-only public registry projection with App Hub destination supplied independently from script host.
- `packages/app-switcher/vite.config.ts` — independent platform asset publication build.
- `packages/app-switcher/hosting/*` — domain-neutral hosting header examples.
- `packages/shared/src/auth/appAuthConfig.ts` — CFR host-scoped preview session; no broad ecosystem cookie-domain assumption.
- `config/solutions.json` — machine-readable CFR + platform integration boundary.
- `docs/PLATFORM_ARCHITECTURE.md` — current architecture overview.
- `docs/integrations/APP_SWITCHER.md` — public cross-platform integration guide.
- `docs/integrations/SSO_FEDERATION.md` — cross-domain identity responsibility boundary.
- `README.md` — current developer/build/deploy entry point.

### 48.12 Acceptance criteria

1. `apps/*` contains only `apps/cfr`.
2. The canonical catalog exists outside `packages/shared` under `packages/app-registry`.
3. CFR App Hub continues to render Your Apps, Available Apps, and Future Apps using the same catalog data through compatibility re-exports.
4. Existing catalog records, descriptions, status data, features, and approved product destinations are preserved.
5. `packages/app-switcher` builds independently from CFR and emits a versioned v1 runtime/catalog pair.
6. CFR Vite configuration no longer imports or compiles App Switcher manifest/catalog assets; any CFR-served launcher files are post-build compatibility copies from the independent platform output.
7. App Switcher runtime does not hardcode the CFR host and loads its default catalog relative to its own hosted script URL.
8. App Hub destination is independently configurable at platform publication time.
9. External HTML integration remains one script + one element.
10. React/TSX consumers retain an optional `.d.ts` typing contract and do not require a Catholic Solutions runtime package.
11. Connected applications may use unrelated domains and repositories without changing Catholic Solutions source architecture.
12. CFR preview session no longer sets a broad `.optioncapp.com` cookie domain by default.
13. Cross-domain SSO is explicitly separated from navigation/launcher behavior.
14. Platform hosting examples define CORS and safe caching for separately hosted switcher assets, while the temporary CFR bridge preserves existing old-path integrations.
15. Root release metadata, `config/solutions.json`, README, architecture/integration docs, and this Living Specification are synchronized to v1.8.0.

### 48.13 Validation record

Validation completed for the delivered v1.8.0 source:

- `apps/*` contains exactly one application workspace: `apps/cfr`.
- The original 18-record catalog body was preserved byte-for-byte after moving it to `packages/app-registry` (except for the type-import path).
- Catalog distribution remains 2 Your Apps, 6 Available Apps, and 10 Future Apps.
- 14 records are launcher-enabled; all 14 publish valid HTTPS destinations and zero invalid launcher URLs were found.
- CFR Vite source contains zero imports of `packages/app-switcher/src/manifest.ts`.
- `app-switcher.js` contains zero embedded product IDs from the registry and passes Node JavaScript syntax validation.
- The manifest projection emits 14 HTTPS launcher records and resolves the configured App Hub URL independently from the script host.
- 44 TypeScript/TSX files across CFR, shared, registry, and App Switcher build source parse with zero TypeScript parser diagnostics.
- Root/package/deployment JSON and IIS XML configuration parse successfully.
- The migration bridge script was executed against representative platform output and copied both versioned and unversioned compatibility assets successfully.
- A dependency-backed TypeScript/Vite production build remains the final CI/local gate because installed workspace dependencies are not included in the supplied source archive and the available npm cache is incomplete.
- Final ZIP integrity is verified before delivery.


## 49. SaaS control-plane architecture — v2.0.0 (August 20, 2026)

This section supersedes §48 where the repository boundary, internal application count, or administration model differs. The v1.6.7 functional behavior of Login, Request Access, App Hub, catalog presentation, external launch behavior, and the universal App Switcher remains the product baseline unless explicitly changed below.

### 49.1 Repository boundary

The Catholic Solutions repository is a SaaS platform/control-plane repository. It contains exactly two deployable React applications:

1. `apps/cfr` — CFR end-user access, central sign-in/request access, and App Hub.
2. `apps/platform-admin` — Super Admin control plane for platform operators.

All business products (OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center, AI tools, and partner SaaS products) are independently deployable applications. Their source code MUST NOT be added under this repository's `apps/*` merely to participate in Catholic Solutions.

### 49.2 Platform packages

- `packages/app-registry` is the framework-neutral single source of application identity, App Hub state, approved destination, and launcher eligibility.
- `packages/app-switcher` is the framework-neutral public launcher implemented as a browser Custom Element with Shadow DOM isolation.
- `packages/shared` is an internal React/auth/design-system package used only by CFR and Super Admin. It MUST NOT become a runtime dependency for external applications.

### 49.3 Super Admin control plane

The Super Admin application is a separate deployment boundary with technical app ID `platform-admin`. It provides the platform-operator surface for registry visibility and future authenticated platform administration. Production mutation actions MUST be implemented through authenticated backend APIs with server-side role enforcement and audit logging; client-side role checks alone are not a security boundary.

The Super Admin prototype MUST expose the centralized registry rather than maintain a second application list.

### 49.4 Independent-domain application model

Every connected business application MAY use its own domain, repository, cloud/provider, deployment pipeline, technology stack, release cadence, data store, and local application session. Catholic Solutions integrates products through stable application IDs and platform contracts rather than shared source code.

Production application destinations MUST be approved HTTPS URLs.

### 49.5 End-user CFR role

CFR remains the user-facing platform entry application and preserves:

- Login / logout / forgot-password / reset-password flow;
- Request Access flow;
- App Hub with Your Apps, Available Apps, and Future Apps;
- catalog-driven Details and Request Access interactions;
- launch behavior derived from the central registry;
- existing premium Catholic Solutions UI/UX direction.

CFR is not the source-code host for connected products.

### 49.6 Universal App Switcher contract

The public integration remains a stable v1 browser contract:
...
External teams MUST NOT need React, Tailwind, Catholic Solutions repository access, copied catalog data, or knowledge of other product URLs. React/TSX projects may add the optional `react.d.ts` declaration for compile-time JSX typing.

### 49.7 Identity and multi-tenant SaaS boundary

Unrelated domains MUST NOT rely on broad shared browser cookies for ecosystem SSO. Production federation SHOULD use the approved standards-based identity provider/OIDC-OAuth contract. Each product owns its local session after federation.

Tenant/organization authorization MUST be enforced by backend services and immutable tenant claims/identifiers. UI filtering is not authorization. Production platform services SHOULD separate identity/access, registry, entitlement/subscription, audit, and workflow/notification responsibilities.

### 49.8 Package/toolchain policy

The release uses the latest stable mutually compatible frontend toolchain validated for this source line. Core baseline: React 19.2.8, React Router DOM 7.18.2, Vite 8.2.1, `@vitejs/plugin-react` 6.0.5, Tailwind CSS 4.3.3, ESLint 10.8.1, and typescript-eslint 8.67.0.

TypeScript 7.0.2 is newer on npm, but the current typescript-eslint v8 line warns when TypeScript 7 is detected. This release intentionally retains TypeScript 6.0.3 until the lint/parser toolchain declares TypeScript 7 support. Dependency freshness MUST NOT override verified compiler/linter compatibility.

### 49.9 Build and deployment separation
...
CFR, Super Admin, and App Switcher are independently buildable/deployable. Business applications have independent repositories and pipelines.

### 49.10 Traceability

- `apps/cfr` — v1.6.7 end-user App Hub/auth baseline retained under the CFR deployment boundary.
- `apps/platform-admin` — new Super Admin control-plane frontend.
- `packages/app-registry/src/appCatalog.ts` — single catalog/launch source.
- `packages/app-switcher` — universal platform-neutral launcher.
- `packages/shared/src/auth/appAuthConfig.ts` — CFR/Admin auth origins and host-scoped preview behavior.
- `packages/shared/src/platform/config/solutionRegistry.ts` — only CFR and Super Admin are internal deployable solutions.
- `config/solutions.json` — machine-readable SaaS repository boundary.
- `docs/PLATFORM_ARCHITECTURE.md` — target SaaS architecture.
- `docs/DEVELOPER_GUIDE.md` — developer workflow/package policy.
- `docs/integrations/APP_SWITCHER.md` — cross-platform integration contract.

### 49.11 Acceptance criteria

1. `apps/*` contains only `cfr` and `platform-admin`.
2. CFR preserves the v1.6.7 Login, Request Access, and App Hub functional baseline.
3. Super Admin reads the same canonical registry and does not maintain a duplicate application list.
4. All connected business applications are modeled as independent-domain products.
5. The App Switcher remains script + custom-element integration for any web platform.
6. Registry data is framework-neutral and shared by CFR/Admin/platform publication.
7. Production destination policy is HTTPS-only.
8. Cross-domain identity is separated from launcher navigation.
9. Root scripts build CFR, Admin, and App Switcher independently and together.
10. Package metadata and lockfile workspace metadata are synchronized to v2.0.0.
11. README, architecture docs, developer guide, integration docs, `config/solutions.json`, and this specification describe the same architecture.

### 49.12 Validation record

Validation completed for the v2.0.0 source delivery:

- `apps/*` contains exactly `cfr` and `platform-admin`.
- The canonical registry preserves 18 application records: 2 Your Apps, 6 Available Apps, and 10 Future Apps.
- 14 launcher-enabled records publish valid HTTPS destinations; zero invalid production launcher URLs were found.
- 49 TypeScript/TSX source/config files parse with zero TypeScript syntax diagnostics.
- Universal App Switcher JavaScript passes Node syntax validation.
- 21 JSON configuration/package files parse successfully.
- Root `package-lock.json` workspace metadata includes CFR, Super Admin, shared, app-registry, and app-switcher at the v2.0.0 architecture boundary.
- Full dependency-backed `npm run typecheck`, lint, and Vite production builds remain the final local/CI gate because this archive does not include installed dependencies and the execution environment cannot fetch the missing npm cache artifacts.


## 50. External SaaS parity with v1.6.7 — v2.0.1 (August 20, 2026)

This amendment preserves the external-application behavior of v1.6.7 inside the v2 SaaS architecture. External SaaS applications remain independently deployed and are linked through the canonical registry; they are not added as source workspaces under `apps/*`.

### 50.1 Functional rules

1. Linked external SaaS products MUST use a stable registry ID and approved HTTPS destination.
2. A partner SaaS product that is available to an organization MUST use `hubSection: 'available'`, `kind: 'external'`, and `launcherEnabled: true` when its destination is approved for publication.
3. Available external SaaS cards preserve the v1.6.7 App Hub interaction: Request access is primary, Details is secondary, and the configured external destination may be opened through the product logo.
4. The public App Switcher MUST publish launcher-eligible **Your Apps + Available Apps only**.
5. Future Apps MUST NOT appear in the App Switcher, even when a deployment URL already exists.
6. External SaaS applications consume the same hosted Web Component contract as first-party independently deployed products and require no Catholic Solutions React/runtime package.
7. The current application remains inert in the launcher when `current-app-id` matches its registry ID; all other approved destinations retain their configured same-tab/new-tab behavior.

### 50.2 Implementation traceability

- `packages/app-registry/src/appCatalog.ts` — canonical linked external SaaS records, v1.6.7-compatible switcher membership, and `linkedExternalSaasApps` projection.
- `packages/shared/src/app/config/appCatalog.ts` — CFR/Admin compatibility re-export.
- `packages/app-switcher/src/manifest.ts` — publishes the registry-projected launcher set without hardcoding SaaS products.
- `apps/cfr/src/modules/appHub/AppCard.tsx` — preserves Request access + Details + logo-link behavior for Available Apps.
- `apps/platform-admin/src/modules/admin/AdminDashboardPage.tsx` — exposes external SaaS/linked state from the same registry.
- `docs/integrations/APP_SWITCHER.md` and `docs/DEVELOPER_GUIDE.md` — external SaaS onboarding contract.

### 50.3 Acceptance criteria

1. The six existing external SaaS Available Apps remain linked by their approved external domains.
2. The App Switcher publishes 8 records: 2 Your Apps + 6 Available external SaaS apps.
3. No Future App is published in the App Switcher.
4. Future records have `launcherEnabled: false` so registry state matches published behavior.
5. External SaaS source code remains outside the Catholic Solutions repository.
6. A new external SaaS product can be onboarded with one registry record and no switcher-code change.
7. Root/package/config/docs version metadata is synchronized to v2.0.1.


## 51. v2.0.2 — External SaaS deployment classification and v1.6.7 catalog parity

**Date:** August 20, 2026. This amendment clarifies the SaaS deployment model without changing the approved v1.6.7 App Hub section behavior.

### 48.1 Requirement

- Every business application represented in the Catholic Solutions catalog MUST be independently deployable outside this repository.
- OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center, AI products and partner products MUST NOT require an `apps/*` source workspace to participate in CFR.
- The registry MUST distinguish deployment location from catalog state using `deploymentModel`, `ownership`, `hubSection`, `launcherEnabled`, `externalUrl`, and `navigationTarget`.
- `deploymentModel: 'external-saas'` MUST be used for all business-product catalog records in this architecture.
- `ownership` MUST distinguish Catholic Solutions-owned products from partner-owned products without changing navigation behavior.

### 48.2 v1.6.7 behavior preserved

- OptionC School and Parish Hub remain **Your Apps**, launcher-enabled, and launch to their approved domains.
- FerrerWorks, Mass Card Requests, Vincent Volunteer, Berchmans, Alive Date and Friar Friend remain **Available Apps**, launcher-enabled, and launch to their approved external domains.
- Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center and other roadmap applications remain **Future Apps** and are not published in the switcher until explicitly promoted.
- Promoting an external SaaS product requires only a central registry state/destination change; product source MUST NOT be added to this repository.

### 48.3 Traceability

- `packages/app-registry/src/types.ts` — deployment and ownership contracts.
- `packages/app-registry/src/appCatalog.ts` — centralized external-SaaS classification and derived collections.
- `packages/app-switcher/src/manifest.ts` — launcher projection from approved Your/Available registry records only.
- `apps/cfr/src/modules/appHub/*` — v1.6.7-compatible App Hub presentation.
- `apps/platform-admin/src/modules/admin/AdminDashboardPage.tsx` — deployment/ownership visibility.
- `docs/DEVELOPER_GUIDE.md` and `docs/PLATFORM_ARCHITECTURE.md` — onboarding and architecture guidance.

### 48.4 Acceptance criteria

1. OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory and Support Center are visible in the registry as external SaaS deployments.
2. No business-product source workspace exists under `apps/*`; only CFR and Platform Admin live there.
3. App Hub sections remain compatible with the v1.6.7 classification.
4. The switcher contains only launcher-approved Your/Available apps with valid destinations.
5. Future Apps never enter the switcher merely because they have a configured URL.
6. Super Admin reports first-party and partner external SaaS separately from current launcher publication.


### 51.5 Validation record

- 18/18 business-product catalog records are explicitly classified with `deploymentModel: 'external-saas'`.
- 12 records are Catholic Solutions-owned first-party SaaS products and 6 are partner-owned SaaS products.
- The v1.6.7-compatible launcher projection contains exactly 8 records: OptionC School, Parish Hub, FerrerWorks, Mass Card Requests, Vincent Volunteer, Berchmans, Alive Date, and Friar Friend.
- The Future Apps collection contains 10 records, including Matt Money, ArcAlerts, Catholic Content, Unified Directory, and Support Center; none is launcher-published.
- `apps/*` contains only `cfr` and `platform-admin`.
- App Registry + App Switcher manifest TypeScript compiles under strict targeted validation; App Switcher runtime passes JavaScript syntax validation; repository JSON files parse successfully.


## 52. v2.0.3 — External SaaS source repository distribution

### Decision

The SaaS architecture separates **deployment/repository ownership** from **catalog membership** without discarding the existing v1.6.7 application source.

- `Catholic_Solutions/` contains CFR, Super Admin, the central registry, the universal App Switcher, and platform-owned shared infrastructure only.
- The actual v1.6.7 source for OptionC School, Parish Hub, Matt Money, ArcAlerts, Catholic Content, Unified Directory, Support Center, and AI Lesson Plan is distributed as standalone sibling projects under `External_SaaS_Apps/` in the release bundle.
- Each extracted project is a repository root, owns its domain/deployment pipeline, and has no source-path dependency on `Catholic_Solutions/packages/shared`.
- A self-contained v1.6.7 compatibility snapshot is retained inside each extracted project only to preserve the existing UI/auth functionality during repository separation. New cross-product integration must use versioned platform contracts.
- The App Switcher is not copied as an application list. External apps load the centralized hosted `app-switcher.js` and render `<catholic-solutions-app-switcher current-app-id="...">`.
- Partner SaaS products whose source never existed in v1.6.7 remain URL-linked registry integrations; source must not be fabricated.

### Acceptance criteria

1. No external business-app source folder exists under `Catholic_Solutions/apps` beyond CFR and Super Admin.
2. The release bundle visibly contains standalone source roots for all business apps that existed as v1.6.7 workspaces.
3. Every extracted app resolves `@shared/*` locally and contains no `../../packages/shared` source dependency.
4. Every extracted app loads the centralized App Switcher runtime once and identifies itself through its stable application ID.
5. Catalog/App Hub state remains centrally governed and compatible with v1.6.7 behavior.
6. Linked partner SaaS with no historical source remains metadata/URL integration only.
